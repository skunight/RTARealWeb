...
exports.viewPackagePriceInput = function(req,res){
    var ret;
        async.waterfall([
            //get list
            function(cb){
                ...
                new httpClient(opt).getReq(function(err,result){
                    if(result.error===0){
                        ...
                        if("input"===req.params.category){
                            ret.proName = "打包产品";
                            ret.modName = "价格录入";
                            res.render("packagePriceInput",ret);
                        }else{
                            ret.proName = "打包产品";
                            ret.modName = "价格审核";
                            res.render("packagePriceAudit",ret);
                        }
                        cb(err,result);
                    }else{
                        throw "error,pls contact admin!";
...
exports.getPackagePriceLogList = function(req,res){
    var params;
    var page = 0;
