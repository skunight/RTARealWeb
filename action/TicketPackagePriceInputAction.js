...
exports.viewTicketPackagePriceInput = function(req,res){
    var ret;
        async.waterfall([
            //get list
            function(cb){
                var opt = {
                    ...
                    path:"/product/ticketPackage/priceLog/list?page=0&status=1",
                    method:"GET"
                };
                ...
                new httpClient(opt).getReq(function(err,result){
                    if(result.error===0){
                        ...
                        if("input"===req.params.category){
                            ret.proName = "打包票务";
                            ret.modName = "价格录入";
                            res.render("ticketPackagePriceInput",ret);
                        }else{
                            ret.proName = "打包票务";
                            ret.modName = "价格审核";
                            res.render("ticketPackagePriceAudit",ret);
                        }
                        cb(err,result);
                    }else{
                        throw "error,pls contact admin!";
...
exports.addInputLog = function(req,res){
    ...
    var opt = {
        hostname:config.inf.host,
        port:config.inf.port,
        path:"/product/ticketPackage/price/create",
        method:"POST"
    };
    try{
...
exports.getTicketPackagePriceLogList = function(req,res){
    var params;
    var page = 0;
    ...
    var opt = {
        hostname:config.inf.host,
        port:config.inf.port,
        path:"/product/ticketPackage/priceLog/list?"+params,
        method:"GET"
    };
